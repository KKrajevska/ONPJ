...
class Model:
    ...
    def construct_LSTM_Model(self, lr):
        ...
        adam=Adam(lr=lr)
        model.compile(loss="binary_crossentropy" if self.encoded_labels.shape[1] == 2 else "categorical_crossntropy", optimizer=adam, metrics=[Accuracy(), Precision(), Recall(), AUC()])
        return model
    
    def train_model(self, lr, epochs, batch_size, task_type):
        my_callbacks = [EarlyStopping(patience=10),
                        ModelCheckpoint(filepath='./models-task-'+task_type+'/model.{epoch:02d}-{val_loss:.2f}.h5'),
                        TensorBoard(log_dir='./logs'),
                        ReduceLROnPlateau(monitor="val_loss",factor=0.5, patience=5, min_lr=10e-5)
                       ]
        model= self.construct_LSTM_Model(lr)
        model.fit(self.train_data, self.encoded_labels, batch_size=batch_size,  validation_split=0.3, epochs=epochs, callbacks=my_callbacks)